...
from example.e14_discriminated_dict_validator import \
    e14_discriminated_dict_validator_print as e14_print
from example.e14_discriminated_dict_validator import \
    e14_discriminated_dict_set
...
E14_CONFIG_BASENAME = 'discriminated_dict_validator.cfg'

E14_SPEC = ExampleProgramSpec(module=e14_module,
                              factory_name='ExampleConfig14',
                              config_factory=ExampleConfig14,
                              set_command=e14_discriminated_dict_set,
                              print_command=e14_print,
                              config_basename=E14_CONFIG_BASENAME)
...
def test_e14_set_uses_defaults_when_no_overrides(
    ...
    with TemporaryDirectory() as dirname:
        ...
        e14_discriminated_dict_set({}, config_file)
...
def test_e14_set_accepts_queue_variant_and_normalizes_values(
    ...
    set_values = cast(SetValues, {
        'export_target': {'kind': 'QUEUE', 'queue': 'ALERTS',
                          'batch_size': 50}})
    with TemporaryDirectory() as dirname:
        config_file = dirname + '/discriminated_dict_validator.cfg'
        e14_discriminated_dict_set(set_values, config_file)
        config = ExampleConfig14(from_json_filename=config_file)
        json_data = read_json_data(config_file)
...
def test_e14_set_accepts_file_variant_without_optional_format(
    ...
    set_values = cast(SetValues, {'export_target': {'kind': 'file',
                                                    'filename': 'report.csv'}})
    with TemporaryDirectory() as dirname:
        config_file = dirname + '/discriminated_dict_validator.cfg'
        e14_discriminated_dict_set(set_values, config_file)
        config = ExampleConfig14(from_json_filename=config_file)
    assert_write_command_output(capsys, config_file)
    assert config.export_target == {'kind': 'file', 'filename': 'report.csv'}
...
def test_e14_print_reports_queue_batch_size_above_maximum(
    ...
    assert_print_validator_error(capsys, monkeypatch, E14_SPEC, {
        'export_target': {
            'kind': 'queue',
            'queue': 'reports',
            'batch_size': 1001
        ...
        'export_target[batch_size]',
        'greater than maximum 1000',
    ])
...
def test_e14_main_round_trip_prints_default_file_target(
    ...
    assert_rt_out(capsys, e14_main, 'discriminated_dict_validator.cfg', [], [
        "Export target: {'filename': 'report.json', 'format': 'json', "
        "'kind': 'file'}", 'Export kind: file', 'Filename: report.json'
    ])
