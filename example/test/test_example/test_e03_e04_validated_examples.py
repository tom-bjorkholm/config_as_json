...
from .helpers import patch_validated_config_stderr
from .helpers import write_validated_example_config_json

EXAMPLE_SPECS = [
    ValidatedExampleSpec(test_id='e03_scalar_validators', module=e03_module,
    ...
    ValidatedExampleSpec(test_id='e04_third_party_class', module=e04_module,
...
def test_validated_example_set_uses_defaults_when_no_overrides(
    ...
    json_text = assert_validated_example_config_written_by_set_command(
        capsys, example_spec, {}, DEFAULT_VALIDATED_EXAMPLE_CONFIG,
        read_json_text=True)
...
def test_validated_example_set_stores_overrides_and_normalizes(
    ...
    json_text = assert_validated_example_config_written_by_set_command(
        capsys, example_spec, set_values, CONFIGURED_VALIDATED_EXAMPLE_CONFIG,
        read_json_text=True)
    assert json_text is not None
...
def test_validated_example_set_reports_validator_error(
        example_spec: ValidatedExampleSpec, capsys: pytest.CaptureFixture[str],
        monkeypatch: pytest.MonkeyPatch) -> None:
    """Show the validator message when writing an invalid value."""
    stderr_buffer = StringIO()
    patch_validated_config_stderr(monkeypatch, example_spec, stderr_buffer)
...
def test_validated_example_print_reports_invalid_stored_value(
    ...
    with TemporaryDirectory() as dirname:
        config_file = dirname + '/' + example_spec.config_basename
        write_validated_example_config_json(config_file,
                                            {'issue_type': 'Feature'})
        example_spec.print_command(config_file)
    assert_validator_error_output(
        capsys, stderr_buffer, ['issue_type', 'not one of the allowed values'])
...
def test_validated_example_main_round_trip_prints_values(
    ...
    expected_lines = [
        'Issue type: Epic', 'Number of iterations: 8', 'Estimate: 8',
        'Severity: WARNING', 'Confidence: 0.8'
    ]
    with TemporaryDirectory() as dirname:
        config_file = dirname + '/' + example_spec.config_basename
        example_spec.main_func([
            'set', '--output', config_file, '--issue-type', 'epic',
            '--number-of-iterations', '8', '--estimate', '8', '--severity',
...
def test_validated_example_main_set_reports_validator_error(
    ...
    with TemporaryDirectory() as dirname:
        config_file = dirname + '/' + example_spec.config_basename
        example_spec.main_func(
            ['set', '--output', config_file, '--estimate', '4'])
    assert_validator_error_output(
        capsys, stderr_buffer, ['estimate', 'not one of the allowed values'])
...
def test_validated_example_main_print_reports_validator_error(
    ...
    with TemporaryDirectory() as dirname:
        ...
        example_spec.main_func(['print', '--input', config_file])
    assert_validator_error_output(
        capsys, stderr_buffer, ['number_of_iterations', 'less than minimum 1'])
