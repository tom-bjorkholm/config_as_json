...
"""Simple example for README_pypi.md with tests."""
...
# pylint: disable=duplicate-code
def test_simple_no_file(capsys: pytest.CaptureFixture[str]) -> None:
...
def test_simple_with_file(capsys: pytest.CaptureFixture[str]) -> None:
...
def test_simple_reads_stored_values(
        capsys: pytest.CaptureFixture[str]) -> None:
    """Test that the simple readme example reads stored values."""
    with TemporaryDirectory() as tmp_dir:
        config_filename = tmp_dir + '/test_config.cfg'
        config = MyConfig()
        config.report_name = 'Sprint report'
        config.story_points = 13
        config.participants = ['Ada', 'Grace']
        config.write(config_filename)
        application(config_filename=config_filename, update_config=False)
    out, err = capsys.readouterr()
...
def test_simple_updates_file(capsys: pytest.CaptureFixture[str]) -> None:
    """Test that the simple readme example can write the read config."""
    with TemporaryDirectory() as tmp_dir:
        config_filename = tmp_dir + '/test_config.cfg'
        config = MyConfig()
        config.report_name = 'Updated report'
        config.story_points = 8
        config.participants = ['Linus']
        config.write(config_filename)
    ...
    assert written_config.participants == ['Linus']
# pylint: enable=duplicate-code
