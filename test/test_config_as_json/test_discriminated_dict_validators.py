...
class _ConstantValidator(MemberValidator):  # pylint: disable=too-few-public-methods # noqa: E501
    ...
    file_variant = DictVariant(
        ...
        rules=[
            ...
            DictRule(
                keys=['format'],
                validators=[StrValidator(
                    allowed_values=['ndjson', 'parquet'],
                    ignore_case=True, normalize=True)])
        ])
    topic_variant = DictVariant(
        mandatory_keys=['topic'],
        allowed_keys=['max_batch'],
        rules=[
            DictRule(
                keys=['topic'],
                validators=[StrValidator(
                    allowed_values=['audit', 'events'],
                    ignore_case=True, normalize=True)]),
            DictRule(
                keys=['max_batch'],
                validators=[IntFloatValidator(
                    min_value=1, max_value=1000, allowed_values=None)])
        ])
    return DiscriminatedDictValidator(
        discriminator_key='kind',
        variants={'disk': file_variant, 'topic': topic_variant},
...
@pytest.mark.parametrize(
    ...
    [(['path', 7], None, (), TypeError,
      'mandatory_keys[1] must be a str'),
     (['path', 'path'], None, (), ValueError,
      "mandatory_keys[1]='path' duplicates an earlier entry"),
     (['path'], ['format', 7], (), TypeError,
      'allowed_keys[1] must be a str'),
     (['path'], ['format', 'format'], (), ValueError,
      "allowed_keys[1]='format' duplicates an earlier entry"),
     (['path'], None, None, TypeError, 'rules must be a sequence'),
     (['path'], None, [object()], TypeError,
      'rules[0] must be a DictRule')])
def test_dict_variant_init_rejects_invalid_arguments(
        mandatory_keys: list[object],
        allowed_keys: Optional[list[object]],
        rules: object,
    ...
    with pytest.raises(exc_type) as exc:
        DictVariant(
            mandatory_keys=cast(Any, mandatory_keys),
...
@pytest.mark.parametrize(
    ...
    [(7, {'file': DictVariant([])}, None, TypeError,
      'discriminator_key must be a str'),
     ('', {'file': DictVariant([])}, None, ValueError,
      'discriminator_key must be non-empty'),
     ('kind', [], None, TypeError, 'variants must be a mapping'),
     ('kind', {}, None, ValueError, 'variants must be non-empty'),
     ('kind', {'file': object()}, None, TypeError,
      "variants['file'] must be a DictVariant"),
     ('kind', {'file': DictVariant([])}, object(), TypeError,
...
def test_discriminated_dict_validator_init_rejects_invalid_arguments(
        discriminator_key: object,
        variants: object,
        discriminator_validator: object,
        exc_type: type[Exception],
        message: str) -> None:
...
def test_discriminated_dict_validator_init_stores_arguments() -> None:
    ...
    discriminator_validator = StrValidator(
        allowed_values=['file'], ignore_case=True, normalize=True)
...
def test_discriminated_dict_validator_normalizes_values_without_mutation(
    ...
    member_value = {'kind': 'DISK', 'path': 'local.parquet',
                    'format': 'NDJSON'}
    result = validate_member(validator, member_value, capsys)
    assert result == {'kind': 'disk', 'path': 'local.parquet',
                      'format': 'ndjson'}
    assert member_value == {'kind': 'DISK', 'path': 'local.parquet',
                            'format': 'NDJSON'}
...
def test_discriminated_dict_validator_accepts_other_variant(
    ...
    result = validate_member(
        validator,
        {'kind': 'TOPIC', 'topic': 'EVENTS', 'max_batch': 50},
        capsys)
    assert result == {'kind': 'topic', 'topic': 'events',
                      'max_batch': 50}
...
def test_discriminated_dict_validator_accepts_omitted_optional_key(
    ...
    validator = make_export_target_validator()
    result = validate_member(
        validator, {'kind': 'topic', 'topic': 'audit'}, capsys)
...
def test_discriminated_dict_validator_allows_kind_in_mandatory_keys(
    ...
    result = validate_member(
        validator, {'kind': 'disk', 'path': 'local.json'}, capsys)
...
def test_discriminated_dict_validator_rejects_invalid_discriminator_value(
    ...
    validator = make_export_target_validator()
    assert_validate_member_failure(
        capsys, validator, {'kind': 'smtp', 'path': 'local.json'},
...
def test_discriminated_dict_validator_rejects_unknown_variant_key(
    ...
    assert_validate_member_failure(
        capsys, validator,
        {'kind': 'disk', 'path': 'local.json', 'topic': 'audit'},
        InvalidConfiguration,
        "Unknown key 'topic' in value")
...
def test_discriminated_dict_validator_rejects_variant_value(
    ...
    assert_validate_member_failure(
        capsys, validator,
        {'kind': 'topic', 'topic': 'audit', 'max_batch': 1001},
        InvalidConfiguration,
...
def test_discriminated_dict_validator_integration_uses_parsed_json(
    ...
    out, err = capsys.readouterr()
    assert config.value == {'kind': 'topic', 'topic': 'events',
                            'max_batch': 7}
    assert out == ''
    assert err == ''
